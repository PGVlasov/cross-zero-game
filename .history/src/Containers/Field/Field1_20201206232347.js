...
function move(userElement, board, row, column) {
  const newBoard = _.cloneDeep(board);
  newBoard[row][column] = userElement;

  return newBoard;
}

function checkWinner(userElement, board, lastRow, lastColumn) {
  const winnerCellNumber = 4;
              ...
              {row.map((cell, columnInd) => (
                <div
                  ...
                  onClick={() => {
                    ...
                    console.log(
                      "Winner? ${}",
                      checkWinner(userElement, board, rowIndex, columnInd)
                    );
