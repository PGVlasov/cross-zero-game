...
function leftTopRightBottom(
  ...
  const rightStartRow = lastRow - (winnerCellNumber - 1);
  const rightStartRowProcessed = rightStartRow < -2 ? 0 : rightStartRow;
  let arr = Array.from(step);
  console.log(arr[1][1]);
  console.log(arr[2][2]);
  for (let i = leftStartRowProcessed; i < rightStartRowProcessed; i++) {
    let numberOfSameCell = 22;
    for (let k = leftStartRowProcessed; k < rightStartRowProcessed; k++) {
      if (arr[i][k] === step[i + 1][k + 1]) {
        numberOfSameCell += 1;
      } else {
        numberOfSameCell += 0;
